...
class Optimizer:
    ...
    def __init__(self, target, objectives=['age', 'loss'], pop_size=constants.POP_SIZE, gens=constants.GENERATIONS,
        ...
        self.NN_seed_weights = NN_seed_weights 
        ...
        self.empowerment_per_gen = np.zeros(self.gens+1)
    ...
    def run(self, continue_from_checkpoint=False, additional_gens=1):
        if not continue_from_checkpoint:
            ...
            new_fits[:self.gen+1] = self.fits_per_gen[:self.gen+1]
            self.fits_per_gen = new_fits

            new_empowerment = np.zeros(max_gens + 1)
            new_empowerment[:self.gen+1] = self.empowerment_per_gen[:self.gen+1]
            self.empowerment_per_gen = new_empowerment

            new_pf_sizes = np.zeros(max_gens + 1)
            new_pf_sizes[:self.gen+1] = self.pf_sizes_per_gen[:self.gen+1]
            self.pf_sizes_per_gen = new_pf_sizes

            if self.triobjective:
                new_obj1_score_per_gen = np.zeros(max_gens + 1)
                new_obj1_score_per_gen[:self.gen+1] = self.obj1_score_per_gen[:self.gen+1]
                self.obj1_score_per_gen = new_obj1_score_per_gen

                new_obj2_score_per_gen = np.zeros(max_gens + 1)
                new_obj2_score_per_gen[:self.gen+1] = self.obj2_score_per_gen[:self.gen+1]
    ...
    def evaluate(self, population):
        for i in range(len(population)):
            population[i].evaluate(self.objectives, target=self.target, g=self.gen)
        return population
    ...
    def save_checkpoint(self):
        filename = '{}_checkpoint_{}gen.p'.format(self.checkpoint_dir, self.gen)
