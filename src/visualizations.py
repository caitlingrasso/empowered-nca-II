...
def save_movie(history, fname, title='', title_color='k', cmap_signal=constants.CMAP_SIGNAL, outline_color='r', original_size=constants.GRID_SIZE, target=None):
    ...
    if target is not None:
        hr_target = convert_to_high_res(target, original_size=original_size)
        target_outline = get_outline(hr_target)
        target_cmap = ListedColormap([[1, 1, 1, 0], constants.TARGET_OUTLINE_COLOR])
    ...
    for t in range(len(history)):
        ...
        if title !='':
            ax.set_title(title, fontsize=20, color=title_color)
